...
def handle_stun_request(server_socket):
    ...
    while True:
        ...
        method, message_length, magic_cookie, transaction_id, attributes = (
            parse_stun_message(data)
        )

        if method == STUN_BINDING_REQUEST:
            print(f"Received Binding Request from {client_address}")
            response = create_stun_binding_response(
                transaction_id, client_address[0], client_address[1]
            )
...
def start_combined_server(host="0.0.0.0", port=3560):
    ...
    while True:
        ...
        threading.Thread(
            target=handle_tcp_health_check, args=(client_socket, client_address)
        ).start()
